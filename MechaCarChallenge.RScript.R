...
# Total summary
Total_summary <- SuspCoil %>% summarize(Mean=mean(PSI), Median=median(PSI), Variance=var(PSI), SD_Engine=sd(PSI), .groups = 'keep')

# Lot summary
lot_summary <- SuspCoil %>% group_by(Manufacturing_Lot) %>% summarize(Mean=mean(PSI), Median=median(PSI), Variance=var(PSI), SD_Engine=sd(PSI), .groups = 'keep')
Total_summary
lot_summary
